...
사용자가 그날의 감정을 선택하고 일기를 기록할 수 있는 웹 서비스입니다.

### 주요 기능

- **일기 CRUD**: `useReducer`와 `Context API`를 활용한 체계적인 일기 데이터 관리.
- **데이터 영속성 (Web Storage)**: `localStorage`를 활용하여 브라우저를 새로고침하거나 종료해도 일기 데이터가 휘발되지 않고 유지되도록 구현했습니다.
    - `JSON.stringify`를 통해 데이터를 저장하고, `JSON.parse`를 통해 초기 데이터를 복원합니다.
